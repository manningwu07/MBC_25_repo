//file: src/app/api/actions/donate-sol-sudan/route.ts
...
export const GET = async (req: Request) => {
  ...
  const response: ActionGetResponse = {
    type: "action",
    icon: `${new URL("/sudan.webp", req.url).toString()}`,
    label: "1 SOL",
    title: "Donate to Sudan",
    description:
      "Donate to Sudan Emergency Relief",
      ...
      actions: [
        {
          ...
          href: `/api/actions/donate-sol-sudan?amount=0.01`,
        },
        {
          type: "transaction",
          label: "0.05 SOL",
          href: `/api/actions/donate-sol-sudan?amount=0.05`,
        },
        {
          type: "transaction",
          label: "0.1 SOL",
          href: `/api/actions/donate-sol-sudan?amount=0.1`,
        },
        {
          // Example for a custom input field
          type: "transaction",
          href: `/api/actions/donate-sol-sudan?amount={amount}`,
          label: "Donate",
          parameters: [
            {
              name: "amount",
...
export const POST = async (req: Request) => {
  try {
    ...
    // We call donate_to_pool for pool id 2 (matches example Anchor code)
    const poolId = 2;
