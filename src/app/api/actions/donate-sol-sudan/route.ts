...
// Forward GET, setting poolId=2 if not provided
export const GET = async (req: Request) => {
  const url = new URL(req.url);
  if (!url.searchParams.has("poolId")) url.searchParams.set("poolId", "2");
  const forwarded = new Request(url.toString(), { method: "GET", headers: req.headers as any });
  return Donate.GET(forwarded);
};

// Forward POST, injecting poolId=2 if not provided
